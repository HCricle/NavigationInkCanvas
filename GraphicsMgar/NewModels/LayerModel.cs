...
using GraphicsMgar.NewModels.LayerDefines;
using System.Numerics;
using GraphicsMgar.NewModels.Helpers;
...
namespace NaiveInkCanvas.Model.NewModels
{
    public class LayerModel: InkCanvasLayer,ILayerCanvas, IGraphicsDraw,ILayerCanSave
    {
        ...
        public LayerModel()
        {
            ...
            KeyDown += LayerModel_KeyDown;
        }

        private void LayerModel_KeyDown(KeyRoutedEventArgs obj)
        {
            GraphicsRects.ForEach(g => g.KeyDown(obj));
        }
        ...
        private async Task<ImgFormat> GetDatas()
        {
            var memory = new InMemoryRandomAccessStream();
            await SaveInMemory(memory, new Size(CanvasSize.Width, CanvasSize.Height));
            return await LayerHelpers.GetImgPixelData(memory);
        }
        /// <summary>
        /// 为了加速，
        /// </summary>
        /// <param name="point"></param>
        /// <param name="colError"></param>
        /// <returns></returns>
    }
